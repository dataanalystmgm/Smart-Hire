...
const regex = /useEffect\(\(\) => \{\s*const analyzePendingCVs = async \(\) => \{[\s\S]*?analyzePendingCVs\(\);\s*\}, \[data, activeTab\]\);/;

const cleanCode = `useEffect(() => {
...
const newContent = content.replace(regex, cleanCode);
